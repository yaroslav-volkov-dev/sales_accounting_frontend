...
const prepareDetailedDataForSelectHandler = <Group extends string>(
  updatedOptionsIds: string[],
  optionsMap: OptionsMap<Group>
): DetailedFiltersState<Group> =>
  updatedOptionsIds.reduce<DetailedFiltersState<Group>>(
    (acc, item) => {
      ...
      if (group)
        acc.groupedFilters[group] = [...(acc.groupedFilters[group] || []), id]
    ...
    },
    {
      allFilters: [],
      ungroupedFilters: [],
      groupedFilters: {} as Record<Group, string[]>,
    }
  )
...
export const FiltersController = <Group extends string>({
  ...
  const onChange = (updatedOptionsIds: string[]) => {
    const data = prepareDetailedDataForSelectHandler(
      updatedOptionsIds,
      optionsMap
    )
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline">
          ...
          <div
            className={cn(
              'w-5 h-5 flex justify-center items-center border rounded text-xs font-semibold',
