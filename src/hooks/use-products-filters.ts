...
const querySchema = z.object({
  [ProductsQueryFilterKey.CATEGORIES_IDS]: z
    ...
    .optional(),
  [ProductsQueryFilterKey.WITHOUT_CATEGORY]: z
    .boolean()
    .optional(),
  [ProductsQueryFilterKey.SUPPLIERS_IDS]: z
    ...
    .transform((val) => (val ? (Array.isArray(val) ? val : [val]) : []))
    .optional(),
  [ProductsQueryFilterKey.WITHOUT_SUPPLIER]: z
    .boolean()
    .optional(),
});
...
export const useProductFiltersState = () => {
  ...
  const state: ProductsFiltersQueryState = parsedFilters.success ? parsedFilters.data : {};
  ...
  const updateQueryState = (updater: (state: ProductsFiltersQueryState) => ProductsFiltersQueryState) => {
    const nextState = updater(state);

    const newQueryParams = queryString.stringify(nextState,
      { encode: false }
    );
  ...
  const updateSupplierFilters = ({ ungroupedFilters, groupedFilters }: {
    groupedFilters: Record<ProductsQueryFilterKey, string[]>;
    ungroupedFilters: string[]
  }) => updateQueryState((state) => ({
    ...state,
